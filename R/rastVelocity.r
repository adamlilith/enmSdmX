...
#' @param x Either a \code{SpatRaster} or a 3-dimensional array. Values \emph{must} be either \code{NA} or >= 0.
#' \itemize{
#' 	\item If \code{x} is a \code{SpatRatster}, then each layer is assumed to represent a time slice. Rasters \emph{must} be in an equal-area projection.
...
#' plot(matsRast, col=c('gray', 'darkgreen'))
#' 
#' v1 <- rastVelocity(mats, times=1:2,
#' latitude=lats, longitude=longs)
#' v2 <- rastVelocity(mats, times=1:2,
...
#' bv1 <- rastVelocity(x=mats, longitude=long, latitude=lat, metrics=metrics,
#' elevation=elev1)
#' 
...
#' elev2[] <- 1
#' elev2[1:2, ] <- 2
#' bv2 <- rastVelocity(x=mats, longitude=long, latitude=lat, metrics=metrics,
#' elevation=elev2)
#'
#' rbind(bv1, bv2)
#' }
...
rastVelocity <- function(
	...
	#########################################################
		...
		# if (any(terra::minValue(x) < 0, na.rm=TRUE) & warn) warning('Negative values appear in "x". Output may be unreliable or undesirable.')
	...
	########################
		...
		# then feed rastVelocity() a subset of rasters corresponding to these indices
